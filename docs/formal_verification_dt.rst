...
* :math:`G(t_1) = [3.0 \leq 5.0] = \text{true}` — **enabled**, produces :math:`A`.
...
* :math:`G(t_2) = [8.0 > 5.0] = \text{true}` — **enabled**, produces :math:`B`.

Exactly one transition fires. :math:`\checkmark`

**Inductive step** :math:`(n = 2 \to n + 1 = 3)`:

The tree grows by splitting the leaf :math:`r_2` (guard :math:`x_1 > 5.0`) on feature :math:`x_2` at threshold :math:`\theta = 4.0`. This replaces :math:`r_2` with two new rules, each inheriting the parent guard:
...
* :math:`G(t_1) = [8.0 \leq 5.0] = \text{false}` — disabled.
* :math:`G(t_2') = [8.0 > 5.0] \wedge [2.0 \leq 4.0] = \text{true} \wedge \text{true} = \text{true}` — **enabled**, produces :math:`B`.
...
* :math:`G(t_3) = [8.0 > 5.0] \wedge [6.0 > 4.0] = \text{true} \wedge \text{true} = \text{true}` — **enabled**, produces :math:`C`.

Exactly one transition fires. :math:`\checkmark`

**Conclusion:** By induction on the number of leaves, every binary split preserves the partition property of the guard set. Therefore, for any Decision Tree with :math:`n` rules, the CPN module :math:`s_{DT}^{(n)}` guarantees that exactly one transition is enabled for every valid input :math:`x \in \mathbb{R}^d`.
